...
function getMessages(app: FastifyInstance, chatId: number, userToken: string, fromLast: number, toLast: number) {
    return app.inject({
        method: 'GET',
        url: `/private/chat/messages/${chatId}/${fromLast}/${toLast}`,
        headers: {
...
describe('Chat test', () => {
    ...
    it('should not send message to someone who blocked', async function (this: any) {
        this.timeout(5000);
        ...
        const blockResponse = await app.inject({
            method: 'POST',
            url: `/private/user/block/${user1id}`,
            headers: {
                'Cookie': `jwt=${token2}`
            }
        });
